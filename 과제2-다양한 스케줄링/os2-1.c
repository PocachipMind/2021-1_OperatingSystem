...
int main(int argc, char* argv[]) {
	...
	while( jobQnum-1 != FinishProcess ) // 잡큐속 프로세스수와 명령어 다 끝난 프로세스 들어온 프로세스 개수가 동일할 때까지
	{
		...
		list_for_each_entry(cur, &job_q, job){
            ...
            { 
                if(cur->pid == 100) // Pid가 100이라면
                {
                    list_add_tail(&cur->Idle,&Idle_q); //Idle큐에 프로세스 추가   
            	}
                else if(cur->pid >= 80) // Pid가 100이 아니고 80 이상이라면
                {
                    list_add_tail(&cur->Real_Time,&Real_Time_q); //Real Time 큐에 프로세스 추가  
                }
                else // 나머지
                {
                    list_add_tail(&cur->Normal,&Normal_q); //Normal Time 큐에 프로세스 추가  
                }
