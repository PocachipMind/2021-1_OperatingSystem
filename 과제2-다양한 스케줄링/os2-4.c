...
int main(int argc, char* argv[]) {
	...
	int jobQnum = answerNum;      //잡큐에 존재하는 프로세스 갯수 ( 나중에 아이들 프로세스 추가될때 +1될 예정)
	...
	else if( answer == 'Y') // 프로세스를 내가 지정해줌
	{
		fprintf(stdout, "\n"); // 보기 이쁘게하려고 
		int i, testAT, testLen;
		int randomAT =0;
		for(i=0; i<jobQnum; i++) 
		{
			next = (process *) malloc(sizeof(process));
			fprintf(stdout, "PDI: %d 프로세스의 arrival_time, 명령어 길이 입력 ( 음수 입력시 랜덤 ): ", i); // 실험 프로세스 랜덤인지 아닌지
			scanf("%d",&testAT);
			scanf("%d",&testLen);
			while(getchar() != '\n');
			next->pid = i;
			next->arrival_time = testAT;
			if( testAT < 0 )
			{
				randomAT += rand()%10;
				next->arrival_time = randomAT;
			}
	...
	jobQnum++; // Idle 프로세스 들어갔으니까 1 추가해줌
	...
	//op.lentgh 백업용 배열
	unsigned char *BackupLen = (unsigned char *) malloc(sizeof(unsigned char)*(jobQnum-1));
	list_for_each_entry(next, &job_q, job) // 큐를 순회하며 명령어 길이 저장
	{
		if(next->pid != 100)
		{
			BackupLen[next->pid] = next->operations[0].len;
		}
	...
	while( answer == 'Y' )
	{
		...
		list_for_each_entry(next, &job_q, job) // 큐를 순회하며 명령어 길이 복원 (스케줄링하며 lentgh 길이 변동했을 수 있음.)
		{
			if(next->pid != 100)
			{
				next->operations[0].len = BackupLen[next->pid];
		...
		while( jobQnum-1 != FinishProcess ) // 잡큐속 프로세스수와 명령어 다 끝난 프로세스 들어온 프로세스 개수가 동일할 때까지
		{
			...
			if(jobQnum-1 == FinishProcess && scheduling == 2)
			{
				break;
			}
