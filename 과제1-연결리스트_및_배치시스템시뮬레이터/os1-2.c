...
int main(int argc, char* argv[]) {
	...
	int readyQnum = 0;    //레디큐에 들어왔다가 종료한 프로세스 개수
	int endoptime = 0;    //명령어 끝나는 clock시간 저장
	bool isworkCPU = false;
	...
	while( jobQnum-1 != readyQnum || endoptime >= clock ) // 잡큐와 레디큐 들어온 프로세스 개수가 동일, 현재 프로세스가 다 끝날때까지,  clock수 늘림
	{
		...
		if( cur->pid != 100 && clock >= endoptime  ) //ready큐 맨 앞 프로세스가 idle 프로세스 아닌것. 단, 현재 실행중인 명령어가 끝난경우만
		{ 
			if( cur->PC < cur->code_bytes/2)// 프로세스 모든 명령어가 끝났는 지 확인
			{
				if(cur->PC == 0 && readyQnum !=0) // 컨텍트 스위치 ( 단일 프로그래밍이므로 0번 명령어 시행시 발생 , 처음 프로세스 접근시 소모 안됨)
				{
					clock += 10;
					idle_time += 10;
					int i;
				...
				{
					endoptime = cur->operations[cur->PC].len + clock;
					fprintf(stdout, "%04d CPU: OP_IO START len: %03d ends at: %04d\n", clock, cur->operations[cur->PC].len, endoptime); //IO 작업 정보 출력
					cur->PC += 1;
					isworkCPU = false;
				}
		...
		}else if( cur->pid == 100 ) // idle 프로세스
		{
			...
			{
				if(jobQnum-1 == readyQnum) // idle을 제외하고 job속 프로세스수와 레디큐에서 완료한 프로세스 수가 동일함
				{
					readyQnum++;
					break;  //모든 프로세스를 다했음. clock소모 없이 그냥 끝내면됨.					
				}
				else if(jobQnum-1 != readyQnum) // job 속 프로세스 수와 레디큐에서 완료한 프로세스 수가 불일치 : 명령어는 끝났으나 도착하지 않은 프로세스 존재
				{
					isworkCPU = false; 
				}
			}
